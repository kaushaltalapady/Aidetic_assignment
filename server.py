# -*- coding: utf-8 -*-
"""
Created on Tue Jun 22 15:23:06 2021
...
def predict():
    ...
    text=''
    for i in input_json.values():
      text=text+i+' '
    text= clean_text(text)
    file =open(r"vector.pkl", "rb")
    vector= pickle.load(file)
    file.close()
    noun_obj=Noun_extractor(vector)
    chunks=noun_obj.get_noun_chunks(text)
    importances=noun_obj.get_importance_of_nouns(chunks)
    output=noun_obj.filter_produce_top_10(importances)
